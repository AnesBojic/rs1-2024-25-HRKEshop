...
namespace RS1_2024_25.API.Endpoints.ImageEndpoints
{
        ...
        .WithResult<List<ImageGetByEntityResponse>>
    {
        ...
        {
            string normalizedType = ImageHelper.Normalize(request.ImageableType);
            ...
            if (!await ImageHelper.IsValidAssociation(db, normalizedType, request.ImageableId, cancellationToken))
            {
                throw new ArgumentException("Entity with given Id does not exist");
            }

            var images = await db.ImagesAll
                .Where(img => img.ImageableId == request.ImageableId && img.ImageableType.ToLower() == normalizedType)
